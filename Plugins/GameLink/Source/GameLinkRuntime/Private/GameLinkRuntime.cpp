...
void FGameLinkRuntimeModule::StartupModule()
{
	...
	*/
	...
	//GetDefault<UGameLinkSettings>() vs GetMutableDefault<UGameLinkSettings>()
...
void FGameLinkRuntimeModule::ReloadModifiedPackages()
{
	...
	*/
	...
	{
		if (bDebugRuntimePackagesReloading)
			UE_LOG(LogGameLinkRuntime, Log, TEXT("Note: FGameLinkRuntimeModule::ReloadModifiedPackages =========>>>>> POST RELOADING CLEANUP <<<<<========="));
		GameLinkProjectSettings->MostRecentModifiedContent.Empty();
		GameLinkProjectSettings->SaveConfig(CPF_Config, *GameLinkProjectSettings->GetDefaultConfigFilename()); //TODO::Test with UpdateDefaultConfigFile() instead, to only modify certain part
	}
